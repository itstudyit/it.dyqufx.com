# 前端要学的测试课 从Jest入门到TDD BDD双实战

## 联系方式

客服微信号：itziyuan_xiaozhi

<img src="https://ziyuanyun.oss-cn-guangzhou.aliyuncs.com/common/20240614073449/666b82192834a.jpg" width="200" height="200" alt="二维码">

## 课程简介

下载链接：https://it.dyqufx.com/posts?id=1066

<img src="https://ziyuanyun.oss-cn-guangzhou.aliyuncs.com/yun/20240515153606/664465e65c69f.jpg" width="500" alt="">

  ┣━mksz372 &#8211; 前端要学的测试课 从Jest入门到TDD BDD双实战

 ┣━课程资料

 ┣━代码资料.zip

 ┣━第6章 Vue中的 BDD 与 集成测试

 ┣━6-1 BDD 的概念以及与 TDD 的对比.mp4

 ┣━6-2 使用 BDD 和集成测试进行 Vuex 项目的测试（1.mp4

 ┣━6-6 路由页面的代码组织.mp4

 ┣━6-3 使用 BDD 和集成测试进行 Vuex 项目的测试（2.mp4

 ┣━6-5 异步测试(2).mp4

 ┣━6-4 异步测试(1).mp4

 ┣━第2章 Jest 前端自动化测试框架基础入门

 ┣━2-12 Jest 中的 Mock（2）.mp4

 ┣━2-4 Jest 的简单配置.mp4

 ┣━2-5 Jest 中的匹配器.mp4

 ┣━2-13 章节小结.mp4

 ┣━2-11 Jest 中的 Mock（1）.mp4

 ┣━2-9 Jest 中的钩子函数mp4.mp4

 ┣━2-8 异步代码的测试方法（2）.mp4

 ┣━2-10 钩子函数的作用域.mp4

 ┣━2-1 自动化测试背景及原理.mp4

 ┣━2-2 前端自动化测试框架 Jest.mp4

 ┣━2-7 异步代码的测试方法（1）.mp4

 ┣━2-6 Jest 命令行工具的使用.mp4

 ┣━2-3 使用 Jest 修改自动化测试样例.mp4

 ┣━第3章 Jest难点进阶

 ┣━3-1 snapshot 快照测试.mp4

 ┣━3-4 ES6 中类的测试.mp4

 ┣━3-2 mock 深入学习.mp4

 ┣━3-3 mock timers.mp4

 ┣━3-5 Jest 中对 DOM 节点操作的测试.mp4

 ┣━第5章 React中的 TDD 与 单元测试

 ┣━5-8 通用代码提取封装.mp4

 ┣━5-15 CodeCoverage 代码覆盖率.mp4

 ┣━5-3 React 环境中配置 Jest（2）.mp4

 ┣━5-2 React 环境中配置 Jest（1）.mp4

 ┣━5-7 Header 组件样式新增及快照测试.mp4

 ┣━5-11 UndoList 样式修饰及测试代码优化.mp4

 ┣━5-9 UndoList 的实现（1）.mp4

 ┣━5-5 使用 TDD 的方式开发 Header 组件 (1).mp4

 ┣━5-1 什么是TDD？（同4-1）.mp4

 ┣━5-16 章节小结.mp4

 ┣━5-4 Enzyme 的配置及使用.mp4

 ┣━5-13 UndoList 编辑功能实现 （2）.mp4

 ┣━5-6 使用 TDD 的方式开发 Header 组件 (2).mp4

 ┣━5-12 UndoList 编辑功能实现 （1）.mp4

 ┣━5-10 UndoList 的实现（2）.mp4

 ┣━5-14 UndoList 编辑功能实现（3）.mp4

 ┣━第1章 前端要学的测试课

 ┣━1-1 前端要学的测试课.mp4

 ┣━第8章 前端自动化测试的思考总结

 ┣━8-1 课程总结.mp4

 ┣━第4章 Vue 中的 TDD 与 单元测试

 ┣━4-1 什么是TDD？.mp4

 ┣━4-12 UndoList 编辑功能实现 (2).mp4

 ┣━4-2 Vue 环境中配置 Jest.mp4

 ┣━4-3 vue-test-utils 的配置及使用.mp4

 ┣━4-5 使用 TDD 的方式开发 Header 组件 (2).mp4

 ┣━4-6 Header 组件样式新增及快照测试.mp4

 ┣━4-14 章节小结mp4.mp4

 ┣━4-7 通用代码提取封装.mp4

 ┣━4-11 UndoList 编辑功能实现(1).mp4

 ┣━4-8 UndoList 的实现（1）.mp4

 ┣━4-4 使用 TDD 的方式开发 Header 组件（1）.mp4

 ┣━4-10 UndoList 样式修饰及测试代码优化.mp4

 ┣━4-9 UndoList 的实现（2）.mp4

 ┣━4-13 CodeCoverage 代码覆盖率.mp4

 ┣━第7章 React中的 BDD 与 集成测试

 ┣━7-1 BDD 的概念以及与 TDD 的对比.mp4

 ┣━7-5 异步测试(2).mp4

 ┣━7-7 路由页面的代码组织.mp4

 ┣━7-4 异步测试(1).mp4

 ┣━7-6 异步测试(3).mp4

 ┣━7-3 使用 BDD 和集成测试进行 Redux 项目的测试（2）.mp4

 ┣━7-2 使用 BDD 和集成测试进行 Redux 项目的测试（1）.mp4

  