# python全栈自动化VIP对标大厂课程

## 联系方式

客服微信号：itziyuan_xiaozhi

<img src="https://ziyuanyun.oss-cn-guangzhou.aliyuncs.com/common/20240614073449/666b82192834a.jpg" width="200" height="200" alt="二维码">

## 课程简介

下载链接：https://it.bcwex.shop/posts?id=3155

<img src="https://ziyuanyun.oss-cn-guangzhou.aliyuncs.com/yun/20240515185029/66449375d7ee0.jpg" width="500" alt="">

  〖课程介绍〗:

 python全栈自动化VIP对标大厂课程(66.79G)

 python全栈自动化VIP对标大厂课程



 〖课程目录〗:

 ├──01_20210401开班典礼&amp;python环境安装

 | ├──python-3.7.4-amd64 .exe 25.44M

 | ├──python及pycharm安装 .docx 12.30kb

 | └──作业 .txt 0.14kb

 ├──02_20210405Python简介&amp;pycharm工具基本使用

 | └──笔记_pycharm工具基本使用及python基本 .pdf 7.78M

 ├──03_20210407Python常见数据类型详解(一)

 | ├──python基本语法及常见数据类型 .pdf 7.81M

 | └──数据类型详解一作业答案 .txt 1.32kb

 ├──04_20210410python数据类型详解(二)

 | ├──Python常见数据类型详解（二） .pdf 7.77M

 | └──源码 .zip 4.96kb

 ├──05_20210412Python数据类型详解（三）

 | ├──dict字典类型 .py 1.47kb

 | ├──list数据类型 .py 1.83kb

 | ├──set集合类型 .py 1.45kb

 | └──类型三作业答案 .txt 1.87kb

 ├──06_Python控制语句if&amp;循环语句

 | ├──06python流程控制IF&amp;循环语句 .pdf 7.76M

 | ├──Code .zip 2.47M

 | └──流程控制语句作业答案 .txt 6.58kb

 ├──07_Python函数（不定长&amp;关键字&amp;默认&amp;必需）

 | ├──07Python函数（必需参数&amp;不定长参数&amp;关键字参数&amp;默认参数） .pdf 255.85kb

 | ├──Code .zip 2.47M

 | └──函数作业答案 .txt 5.08kb

 ├──08_函数嵌套&amp;递归函数&amp;文件操作

 | ├──07函数嵌套&amp;递归函数&amp;文件操作 .pdf 7.73M

 | └──Code .zip 2.47M

 ├──09_时间日期模块&amp;异常处理

 | ├──09时间日期模块&amp;异常处理 .pdf 7.87M

 | └──Code .zip 2.48M

 ├──10_面向对象（类&amp;对象&amp;属性&amp;方法）

 | ├──10面向对象编程(类&amp;对象&amp;属性&amp;方法) .pdf 7.79M

 | └──Code .zip 2.48M

 ├──11_面向对象编程(一)

 | ├──11面向对象编程（一） .pdf 7.76M

 | ├──Code .zip 2.49M

 | └──作业答案_面向对象1 .txt 1.51kb

 ├──12_面向对象（封装&amp;继承&amp;多态&amp;多态）

 | ├──Code .zip 2.50M

 | ├──附加作业 .txt 0.89kb

 | ├──课堂笔记_1面向对象编程-封装&amp;继承&amp;重载&amp;多态 .pdf 7.81M

 | ├──某商城某天的购物数据 .jpg 63.87kb

 | └──作业答案_面向对象2 .txt 2.95kb

 ├──13_配置文件ini及YAML格式文件处理

 | ├──13配置文件ini及YAML格式文件处理 .pdf 7.76M

 | └──Code .zip 2.70M

 ├──14_Excel文件处理及日志收集

 | ├──14excel文件处理及日志处理 .pdf 7.78M

 | ├──Code .zip 3.34M

 | └──作业_excel文件处理及日志处理 .txt 0.50kb

 ├──15_Python操作mysql数据库

 | ├──15python操作mysql数据库 .pdf 292.74kb

 | ├──Code .zip 3.34M

 | └──mysql安装步骤 .txt 2.79kb

 ├──16_Python装饰器&amp;反射应用及原理详解

 | ├──16详解装饰器&amp;反射原理及应用 .pdf 7.84M

 | ├──Code .zip 3.35M

 | └──作业答案 .txt 1.51kb

 ├──17_接口01_精通Postman接口测试基础应用

 | ├──今日内容：基于工具Postman的接口自动化基础应用以及接口关联 .pdf 7.97M

 | ├──微信公众号接口测试用例模板 .xlsx 12.27kb

 | └──作业： .pdf 169.09kb

 ├──18_接口02_精通Postman接口测试

 | ├──16.jenkins持续集成环境从0到1搭建全过程 .pdf 700.21kb

 | └──今日内容：精通Postman接口测试之接口关联，动态参数，断言以及Postman+Newman+Jenkins持续集成 .pdf 7.90M

 ├──19_接口03_精通Postman接口测试

 | ├──api_login .py 2.94kb

 | └──今日内容：精通Postman接口测试之接口鉴权，接口Mock，接口加解密以及接口签名Sign .pdf 317.03kb

 ├──20_接口04_接口自动化框架封装思想建议之httprunner(上)

 | ├──edit_flag .har 3.73kb

 | ├──get_token .har 1.90kb

 | ├──select_flag .har 7.53kb

 | └──今日内容：接口自动化框架封装思想建立之httprunner框架（上） .pdf 323.29kb

 ├──21_接口05_接口自动化框架封装思想建立之httprunner（中）

 | └──今日内容：接口自动化框架封装思想建立之httprunner框架（中） .pdf 7.82M

 ├──22_接口06_接口自动化框架封装思想建立之httprunner（下）

 | ├──allure-2.13.7 .zip 16.44M

 | └──今日内容：接口自动化框架封装思想建立之httprunner框架（下） .pdf 288.46kb

 ├──23_接口07_用例管理框架之pytest单元测试框架（上）

 | └──用例管理框架之pytest单元测试框架（上） .pdf 7.91M

 ├──24_接口08_用例管理框架pytest之fixtrue,conftest.py,allure报

 | ├──logo .png 76.60kb

 | └──今日内容：用例管理框架pytest之fixtrue,conftest.py,allure报告以及logo定制（上） .pdf 333.68kb

 ├──25_接口09_用例管理框架pytest之allure报告定制以及数据驱动(下)

 | ├──pytestdemo .rar 8.46M

 | └──今日内容：用例管理框架pytest之allure报告定制以及数据驱动 .pdf 7.90M

 ├──26_接口10_接口自动化之requests模块详解以及Cookie，Session关联处理

 | └──接口自动化之requests模块详解以及Cookie，Session关联处理 .pdf 7.83M

 ├──27_web自动化_20210629_Web自动化之HTML&amp;JavaScript详解

 | ├──a .js 0.04kb

 | ├──frame .html 0.22kb

 | ├──js .html 1.76kb

 | ├──npp.7.8.8.Installer.x64 .exe 3.86M

 | └──Web自动化之HTML&amp;JavaScript详解 .pdf 393.96kb

 ├──28_web自动化_20210703_Selenium及Selenium原理及源码讲解

 | ├──02_210703selenium webdriver环境搭建及原理讲解 .pdf 434.00kb

 | └──js .html 3.33kb

 ├──29_web自动化_20210705_浏览器常用操作及元素定位

 | └──selenium常用浏览器操作及元素定位 .pdf 218.23kb

 ├──30_web自动化_20210710_web常用操作(二)

 | └──05_210710selenium常用操作（二） .pdf 227.23kb

 ├──31_web自动化_20210712_元素常用操作(一)

 | └──04_210707_selenium元素常见操作(一) .pdf 248.86kb

 ├──32_web自动化_元素常用操作(三)

 | ├──06_210712_web页面常用操作(三) .pdf 228.74kb

 | ├──WebCode .rar 94.13kb

 | └──WinSpy17 .zip 58.33kb

 ├──33_web自动化实施&amp;unittest应用

 | └──07_web自动化实施&amp;unittest框架 .pdf 280.67kb

 ├──34_Web自动化_Unittest框架应用

 | ├──08_unittest的应用 .pdf 236.76kb

 | └──HTMLTestRunner .py 23.90kb

 ├──35_Web自动化_自动化测试框架之POM设计模式详解

 | └──09_自动化测试框架之pom设计模式详解 .pdf 266.50kb

 ├──36_Web自动化_自动化测试框架封装(二)

 | ├──210726_数据驱动及关键字驱动 .pdf 164.23kb

 | └──WebCode .rar 1.19M

 ├──37_20210726_Web自动化_自动化测试框架封装(三)

 | └──WebCode .rar 1.19M

 ├──38_web自动化_web自动化测试框架封装之数据驱动&amp;关键字驱动

 ├──39_web自动化_基于关键字驱动+数据驱动+反射框架封装（二）

 ├──40_app自动化_app环境搭建及appium运行原理及常见abd命令

 | ├──App环境搭建&amp;Appium运行原理详解 .pdf 346.50kb

 | ├──APP自动化测试环境搭建 .pdf 8.37M

 | ├──dr.fone3.2.0 .apk 7.66M

 | ├──kaoyan3.1.0 .apk 9.25M

 | ├──mymoney .apk 22.46M

 | └──wandoujia1 .apk 12.82M

 ├──41_App元素定位工具及App元素定位方式

 | ├──02Appium常用元素定位工具及元素定位方法 .pdf 336.22kb

 | └──appcode .rar 5.57kb

 ├──42_App自动化_Appium滑屏&amp;触屏&amp;多点触控操作

 | ├──appcode .rar 8.84kb

 | └──APP自动化之Appium多点触控&amp;列表滑动&amp;Touchaction应用 .pdf 411.96kb

 ├──43_App自动化测试_Appium多点触控及H5元素

 | ├──04多点触控&amp;H5元素定位 .pdf 464.80kb

 | ├──appcode .rar 11.14kb

 | └──uc-devtools-msi_0.4.1 .zip 57.54M

 ├──44_App自动化测试_App自动化框架封装

 | ├──05App自动化测试框架封装 .pdf 177.77kb

 | └──appcode .rar 20.48kb

 ├──45_20210814_APP自动化测试_Appium之微信小程序&amp;微信公众号自动化测试

 | ├──06App自动化测试之微信小程序 .pdf 264.47kb

 | └──appcode .rar 21.96kb

 ├──46_测开01_Git版本控制以及基于Flask的Mock Server服务器

 | ├──2.GIT分布式版本控制系统的安装和配置 .pdf 8.40M

 | └──今日内容：Git，GitHub，Gitee集成Pycharm实现代码版本控制以及基于Flask的接口Mock Server服务器 .pdf 7.95M

 ├──47_测开02_Jenkins持续集成CI和持续部署CD

 | ├──16.jenkins持续集成环境从0到1搭建全过程 .pdf 704.53kb

 | ├──Jenkins邮件标题和邮件内容模板 .txt 2.04kb

 | └──今日内容：Jenkins持续集成CI，持续部署CD，Allure报告集成以及发送电子邮件 .pdf 7.95M

 ├──48_测开03_Docker容器技术

 | └──今日内容：Docker容器技术介绍，应用场景，安装应用以及项目部署 .pdf 7.90M

 ├──49_侧开之简历制作，面试技巧！ (1)

 | └──今日内容：BATJ一线互联网大厂简历制作和面试技巧 .pdf 258.33kb

 ├──49_侧开之简历制作，面试技巧！

 | └──今日内容：BATJ一线互联网大厂简历制作和面试技巧 .pdf 258.33kb

 ├──MobileFile

 | ├──Image

 | ├──mpFile

 | └──thumb

 ├──MobileFile

 | ├──Image

 | ├──mpFile

 | └──thumb

 ├──19-10-31-14-43-54 .mp4 30.15M

 ├──case_login_nopasswd .py 3.08kb

 ├──debugtalk .py 0.22kb

 ├──FiddlerSetup_5.0.20204.45441 .exe 6.34M

 ├──jdk-8u231-windows-x64 .exe 210.18M

 ├──jetbrains-agent-latest .zip 2.35M

 ├──npp.7.8.8.Installer.x64 .exe 3.86M

 ├──test_user_login .py 2.36kb

 └──报错信息 .txt 1.72kb

 1-开班典礼&amp;Python环境搭建&amp;Pycharm基本使用 .mp4 1.30G

 10-BATJ简历编写，面试技巧&amp;精选面试题讲解 .mp4 1.84G

 2-1Python基础语法&amp;数值类型及运算符 .mp4 1.88G

 2-2python常见数据类型详解（一） .mp4 545.85M

 2-3python常见数据类型详解（二） .mp4 1.08G

 2-4python常见数据类型详解(三) .mp4 1012.11M

 2-5Python流程控制之if&amp;（for-while）循环语句 .mp4 1.16G

 3-10python装饰器&amp;反射原理应用和详解 .mp4 1.30G

 3-1Python函数及参数详解(关键字-默认-不定长) .mp4 1.00G

 3-2函数递归(嵌套)&amp;文件&amp;OS模块 .mp4 1.06G

 3-3时间模块&amp;异常处理 .mp4 1.25G

 3-4面向对象编程（类，对象，属性，方法） .mp4 2.03G

 3-5面向对象编程(一) .mp4 1.17G

 3-6面向对象编程（封装&amp;继承&amp;重载&amp;多态） .mp4 929.94M

 3-7全局配置INI文件&amp;YAML文件处理 .mp4 924.13M

 3-8python操作EXCEL文件&amp;python日志收集处理 .mp4 1.12G

 3-9Python操作数据库之pymysql应用 .mp4 978.25M

 4-1Postman基础应用-接口关联以及接口协议 .mp4 1.46G

 4-2Postman动态参数-断言以及接口加密实战 .mp4 1.17G

 4-3Postman接口签名和Cookie-SessionToken鉴权 .mp4 1.41G

 4-4HttpRunner快速上手-接口关联及函数热加载 .mp4 780.01M

 4-5HttpRunner关键字详解及测试用例分层设计 .mp4 1000.73M

 4-6HttpRunner数据驱动及接口自动化框架封装 .mp4 1.14G

 5-1接口框架项目实战(一)：pytest运行规则夹具 .mp4 885.75M

 5-2接口框架项目实战(二)：fixture和conftest .mp4 1.15G

 5-3接口框架项目实战(三)：allure和数据驱动 .mp4 1.35G

 5-4接口框架项目实战(四)：requests详解和封装 .mp4 932.80M

 5-5接口框架项目实战(五)：请求和接口关联封装 .mp4 1.46G

 5-6接口框架项目实战(六)：分析YAML用例封装 .mp4 1.26G

 5-7接口框架项目实战(七)：热加载以及断言封装 .mp4 1.45G

 5-8接口框架项目实战(八)：CSV数据驱动封装 .mp4 1.44G

 6-1Web自动化之HTML&amp;JavaScript详解 .mp4 1.22G

 6-2接口框架项目实战(九)：异常日志和基础路径 .mp4 1.59G

 6-3Selenium环境搭建及Selenium原理讲解 .mp4 907.86M

 6-4Selenium元素定位&amp;Xpath定位&amp;CSS定位 .mp4 1.73G

 6-5Web页面操作(一)：四大操作&amp;等待&amp;三大切换 .mp4 1.10G

 6-6Web页面操作(二)：下拉&amp;鼠标键盘&amp;文件上传 .mp4 841.43M

 7-1Web框架项目实战(一)：自动化场景&amp;需求分析 .mp4 991.22M

 7-2Web框架项目实战(二)：Unittest四大组件 .mp4 1.48G

 7-3Web框架项目实战(三)：Unittest框架应用 .mp4 966.90M

 7-4Web框架项目实战(四)：PO设计模式详解 .mp4 836.48M

 7-5Web框架项目实战(五)：关键字驱动设计模式 .mp4 1.48G

 7-6Web框架项目实战(六)：DDT数据驱动详解 .mp4 977.44M

 7-7Web框架项目实战(七)：复杂用例&amp;截图&amp;日志 .mp4 1.06G

 7-8基于关键字+数据驱动+反射自动化框架搭建 .mp4 1.66G

 7-9基于关键字+数据驱动自动化框架搭建（二） .mp4 1.13G

 8-1APP环境搭建&amp;Appium运行原理详解 .mp4 1.27G

 8-2Appium日志解析&amp;三大元素工具&amp;元素定位 .mp4 984.69M

 8-3Appium多点触控&amp;列表滑动&amp;touchaction应用 .mp4 1.17G

 8-4APP框架项目实战(一)：H5混合应用自动化 .mp4 994.30M

 8-5APP框架项目实战(二)：微信小程序自动化 .mp4 984.04M

 8-6APP框架项目实战(三)：APP自动化框架整合 .mp4 1.11G

 9-1GIT，GITHub集成Pycharm&amp;Flask的Mock服务 .mp4 1.08G

 9-2Jenkins安装部署，JOB，插件，权限持续集成 .mp4 903.74M

 9-3Docker容器安装，配置，Nginx，集成Jenkins .mp4 1018.35M

 9-4接口+UI自动化测试框架一体化平台搭建 .mp4 1.01G

 211qq群笔记下载 .rar 625.09M

  