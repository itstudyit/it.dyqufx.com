# 【中级】C++项目-手写移植Nginx和SGI STL内存池源码

## 联系方式

客服微信号：itziyuan_xiaozhi

<img src="https://ziyuanyun.oss-cn-guangzhou.aliyuncs.com/common/20240614073449/666b82192834a.jpg" width="200" height="200" alt="二维码">

## 课程简介

下载链接：https://it.dyqufx.com/posts?id=2912

<img src="https://ziyuanyun.oss-cn-guangzhou.aliyuncs.com/yun/20240515184429/6644920d56576.jpg" width="500" alt="">

  

 ——/【中级】C++项目-手写移植Nginx和SGI STL内存池源码/

 ├──内存池源码剖析

 | ├──01 source insight工具的使用介绍

 | | └──source insight工具的使用介绍.mp4 180.45M

 | ├──02 SGI STL的vector容器源码如何管理对象

 | | └──SGI STL的vector容器源码如何管理对象.mp4 130.47M

 | ├──03 SGI STL二级空间配置器重要成员解读

 | | └──SGI STL二级空间配置器重要成员解读.mp4 267.59M

 | ├──04 两个重要的辅助函数_S_round_up和_S_freelist_index代码解读

 | | └──_S_round_up和_S_freelist_index代码解读.mp4 85.66M

 | ├──05 allocate内存分配函数代码解读

 | | └──allocate内存分配函数代码解读.mp4 122.34M

 | ├──06 _S_refill函数代码解读

 | | └──_S_refill函数代码解读.mp4 174.04M

 | ├──07 _S_chunk_alloc函数代码解读 &#8211; 1

 | | └──_S_chunk_alloc函数代码解读 &#8211; 1.mp4 171.00M

 | ├──08 deallocate内存释放函数代码解读

 | | └──deallocate内存释放函数代码解读.mp4 55.36M

 | ├──09 _S_chunk_alloc函数代码解读 &#8211; 2

 | | └──_S_chunk_alloc函数代码解读 &#8211; 2.mp4 251.72M

 | ├──10 _S_chunk_alloc函数代码解读 &#8211; 3

 | | └──_S_chunk_alloc函数代码解读 &#8211; 3.mp4 305.26M

 | ├──11 _S_chunk_alloc函数代码解读 &#8211; 4

 | | └──_S_chunk_alloc函数代码解读 &#8211; 4.mp4 167.65M

 | ├──12 reallocate内存扩容函数代码解读

 | | └──reallocate内存扩容函数代码解读.mp4 123.83M

 | ├──13 nginx内存池创建函数代码解读

 | | └──nginx内存池创建函数代码解读.mp4 241.36M

 | ├──14 nginx内存池分配方案-小块内存分配代码解读

 | | └──小块内存分配代码解读.mp4 210.55M

 | ├──15 nginx内存池分配方案-大块内存分配释放代码解读

 | | └──大块内存分配释放代码解读.mp4 130.03M

 | ├──16 nginx内存池重置函数和小块内存回收方案代码解读

 | | └──nginx内存池重置函数和小块内存回收方案.mp4 148.86M

 | ├──17 nginx内存池外部资源释放和内存池销毁代码解读

 | | └──nginx内存池外部资源释放和内存池销毁.mp4 222.99M

 | ├──18 nginx源码编译测试内存池接口函数的功能

 | | └──nginx源码编译测试内存池接口函数的功能.mp4 312.91M

 | ├──19 跟着做nginx内存池源码移植项目 &#8211; 1

 | | └──跟着做nginx内存池源码移植项目 &#8211; 1.mp4 349.52M

 | ├──20 跟着做nginx内存池源码移植项目 &#8211; 2

 | | └──跟着做nginx内存池源码移植项目 &#8211; 2.mp4 362.55M

 | ├──21 跟着做nginx内存池源码移植项目 &#8211; 3

 | | └──跟着做nginx内存池源码移植项目 &#8211; 3.mp4 42.24M

 | ├──22 跟着做SGI STL内存池源码移植项目 &#8211; 1

 | | └──跟着做SGI STL内存池源码移植项目 &#8211; 1.mp4 310.02M

 | └──23 跟着做SGI STL内存池源码移植项目 &#8211; 2

 | | └──跟着做SGI STL内存池源码移植项目 &#8211; 2.mp4 277.74M

 └──资料

 | ├──nginx-1.12.2.tar.gz 958.68kb

 | ├──nginx-release-1.13.1.zip 1.18M

 | ├──nginx代码测试编译链接命令.txt 0.23kb

 | ├──nginx内存池代码剖析上课图例.png 1020.31kb

 | ├──nginx内存池源码剖析.pdf 290.52kb

 | ├──ngx_testpool.c 1.47kb

 | ├──sgi stl.zip 216.68kb

 | ├──SGI STL二级空间配置器源码剖析.pdf 332.03kb

 | ├──SGI STL内存池源码讲解上课图示.png 742.34kb

 | ├──sourceinsight4.0.rar 20.96M

 | └──推荐课程学习路径.pdf 95.50kb

  