# React17+React Hook+TS4 最佳实践仿 Jira 企业级项目[完结]

## 联系方式

客服微信号：itziyuan_xiaozhi

<img src="https://ziyuanyun.oss-cn-guangzhou.aliyuncs.com/common/20240614073449/666b82192834a.jpg" width="200" height="200" alt="二维码">

## 课程简介

下载链接：https://it.bcwex.shop/posts?id=970

<img src="https://ziyuanyun.oss-cn-guangzhou.aliyuncs.com/yun/20240515153444/664465949a167.jpg" width="500" alt="">

  ┣━mksz482 &#8211; React17+React Hook+TS4 最佳实践仿 Jira 企业级项目[完结]

 ┣━第2章 项目起航：项目初始化与配置

 ┣━2-2 配置 eslint、 prettier 和 commitlint 规范工程.mp4

 ┣━2-3 对比常见 Mock 方案 配置 JSON SERVER.mp4

 ┣━2-1 用 Create React App 初始化项目.mp4

 ┣━第14章 自动化测试

 ┣━14-2 传统单元测试.mp4

 ┣━14-3 自动化测试 hook.mp4

 ┣━14-4 自动化测试组件.mp4

 ┣━14-1 自动化测试简介.mp4

 ┣━第6章 CSS 其实很简单 &#8211; 用 CSS-in-JS 添加样式

 ┣━6-1 安装与使用 antd 组件库.mp4

 ┣━6-7 清除前面课程留下的警告信息.mp4

 ┣━6-6 完善项目列表页面样式.mp4

 ┣━6-2 为什么我们需要CSS-in-JS方案 -通过传统CSS缺陷学习CSS-in-JS必要性.mp4

 ┣━6-4 用Grid和Flexbox布局优化项目列表页面.mp4

 ┣━6-3 最受欢迎的CSS-in-JS方案 &#8211; Emotion的安装与使用.mp4

 ┣━6-5 用CSS-in-JS状态工程创建自定义组件：Row组件实现.mp4

 ┣━第13章 React 的性能优化

 ┣━13-2 用React.memo优化组件性能.mp4

 ┣━13-1 用代码分割优化性能.mp4

 ┣━课程资料

 ┣━资料.zip

 ┣━第5章 JWT、用户认证与异步请求

 ┣━5-4 用useContext存储全局用户信息.mp4

 ┣━5-3 JWT原理与auth-provider实现.mp4

 ┣━5-9 TS的Utility Types-Pick、Exclude、Partial和Omit实现.mp4

 ┣━5-5 用useAuth切换登录与非登录状态.mp4

 ┣━5-2 连接真实服务端 &#8211; 专属开发者⼯具介绍与安装.mp4

 ┣━5-6 用fetch抽象通用HTTP请求方法，增强通用性.mp4

 ┣━5-7 用useHttp管理JWT和登录状态，保持登录状态.mp4

 ┣━5-1 用React表单、TS的类型继承和鸭子类型实现登录表单.mp4

 ┣━5-8 TS的联合类型、Partial和Omit介绍.mp4

 ┣━第3章 React 与 Hook 应用：实现项目列表

 ┣━3-3 学习自定义Hook &#8211; 用useDebounce减少工程搜索请求频率.mp4

 ┣━3-2 用状态提升分享组件状态，完成工程列表页面.mp4

 ┣━3-1 用 JSX 列表渲染开发工程列表页面.mp4

 ┣━第12章 看板页面及任务组页面开发

 ┣━12-11 拖拽实现(下).mp4

 ┣━12-1 解决前面的 3 个 Bug.mp4

 ┣━12-4 添加task, bug 图标.mp4

 ┣━12-10 拖拽实现(上).mp4

 ┣━12-17 完成popover.mp4

 ┣━12-3 看板列表初步开发.mp4

 ┣━12-2 看板列表开发准备工作.mp4

 ┣━12-8 编辑任务功能.mp4

 ┣━12-12 拖拽持久化(上).mp4

 ┣━12-13 拖拽持久化 (下).mp4

 ┣━12-16 任务组页面(下).mp4

 ┣━12-5 添加任务搜索功能.mp4

 ┣━12-14 排序乐观更新.mp4

 ┣━12-9 看板和任务删除功能.mp4

 ┣━12-6 优化看板样式.mp4

 ┣━12-18 开发完成，部署页面.mp4

 ┣━12-7 创建看板与任务.mp4

 ┣━12-15 任务组页面 (上).mp4

 ┣━第1章 课程介绍(了解本课程必看)

 ┣━1-1 课程导学.mp4

 ┣━第7章 用户体验优化 &#8211; 加载中和错误状态处理

 ┣━7-1 给页面添加Loading和Error状态，增加页面友好性.mp4

 ┣━7-5 实现Error Boundaries，捕获边界错误.mp4

 ┣━7-2 用高级 Hook-useAsync统一处理Loading和Error状态.mp4

 ┣━7-3 登录注册页面Loading和Error状态处理，与Event Loop详解.mp4

 ┣━7-4 用useAsync获取用户信息.mp4

 ┣━第11章 用 react-query 获取数据，管理缓存

 ┣━11-2 用 react-query 来处理 -服务端缓存-.mp4

 ┣━11-5 编辑和添加工程功能(下).mp4

 ┣━11-1 用url参数管理项目模态框状态.mp4

 ┣━11-6 用 react-query 实现乐观更新.mp4

 ┣━11-4 编辑和添加工程功能(上).mp4

 ┣━11-9 跨组件状态管理方案总结.mp4

 ┣━11-8 修复url多余参数.mp4

 ┣━11-3 类型守卫，用useQuery缓存工程列表.mp4

 ┣━11-7 抽象乐观更新通用hook.mp4

 ┣━第9章 用户选择器与项目编辑功能

 ┣━9-3 用 useEditProject 编辑项目.mp4

 ┣━9-5 完成编辑后刷新功能.mp4

 ┣━9-1 实现id-select.tsx解决id类型 难题.mp4

 ┣━9-2 抽象user-select组件选择用户.mp4

 ┣━9-4 编辑后刷新-useState的懒初始化与保存函数状态.mp4

 ┣━第10章 深入React 状态管理与Redux机制

 ┣━10-11 应用redux-toolkit管理模态框.mp4

 ┣━10-10 配置redux-toolkit.mp4

 ┣━10-6 redux用法介绍.mp4

 ┣━10-1 useCallback应用，优化异步请求.mp4

 ┣━10-2 状态提升，组合组件与控制反转(上).mp4

 ┣━10-3 状态提升，组合组件与控制反转(下).mp4

 ┣━10-9 为什么我们需要redux-thunk？.mp4

 ┣━10-12 用redux-thunk管理登录状态.mp4

 ┣━10-7 react-redux 与 HoC.mp4

 ┣━10-5 用useReducer进行状态管理.mp4

 ┣━10-4 合并组件状态，实现useUndo.mp4

 ┣━第4章 TS 应用：JS神助攻 &#8211; 强类型

 ┣━4-6 作业解答 &#8211; ⽤ Hook + TS + TS泛型实现useArray.mp4

 ┣━4-1 为什么我们需要TS &#8211; 真实场景学习 TS 的必要性.mp4

 ┣━4-4 学习泛型，用泛型增强useDebounce类型灵活性.mp4

 ┣━4-3 TS知识梳理、总结与提高.mp4

 ┣━4-5 作业练习 &#8211; 用 Hook + TS + TS泛型实现useArray.mp4

 ┣━4-2 将项目列表页面JS改造成TS，增强类型，减少Bug.mp4

 ┣━第8章 Hook，路由，与 URL 状态管理

 ┣━8-1 ⽤useRef实现useDocumentTitle &#8211; useRef与Hook 闭包详解（上）.mp4

 ┣━8-6 用useMemo解决依赖循环问题 &#8211; Hook的依赖问题详解.mp4

 ┣━8-4 添加看板和任务组路由.mp4

 ┣━8-5 初步实现 useUrlQueryParam 管理 URL 参数状态.mp4

 ┣━8-3 添加项目列表和项目详情路由.mp4

 ┣━8-7 完成URL状态管理与JS中的 iterator讲解.mp4

 ┣━8-2 ⽤useRef实现useDocumentTitle &#8211; useRef与Hook 闭包详解（下）.mp4

  